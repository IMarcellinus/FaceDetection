import cv2, os, numpy as np, time
wajahDir = 'datawajah'
latihDir = 'latihwajah'
cam = cv2.VideoCapture(0)
cam.set(3, 640)  # ubah lebar cam code 3 dan 640 itu ukuran
cam.set(4, 480)  # ubah tinggi cam mode 4 dan 480 itu ukuran
faceDetector = cv2.CascadeClassifier('deteksiMuka.xml')
faceRecognizer = cv2.face.LBPHFaceRecognizer_create()

faceRecognizer.read(latihDir+'/training.xml')
font = cv2.FONT_HERSHEY_COMPLEX

id = 0
names = ['Tidak Diketahui','Marcel']

minWidth = 0.1*cam.get(3)
minHeight = 0.1*cam.get(4)

while True:
    retV, frame = cam.read()
    frame = cv2.flip(frame, 1) # vertical_flip
    abuAbu = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    faces = faceDetector.detectMultiScale(
        abuAbu, 1.2, 5,minSize=(round(minWidth),round(minHeight)),)  # frame, scaleFactor, minNumbers
    for (x, y, w, h) in faces:
        frame = cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)
        id, confidence = faceRecognizer.predict(abuAbu[y:y+h,x:x+w]) # confidence = 0 artinya cocok sempurna
        if confidence<=40 :
            nameID = names[id]
            confidenceTxt = "{0}%".format(round(100-confidence))
        else:
            nameID = names[0]
            confidenceTxt = "{0}%".format(round(100-confidence))
        cv2.putText(frame,str(nameID),(x+5,y-5),font,1,(255,255,0),2)
        cv2.putText(frame,str(confidenceTxt), (x+5, y+h-5),font, 1, (255, 255, 0), 1)

    cv2.imshow('Absen Mahasiswa', frame)
    # cv2.imshow('Webcamku 2', abuAbu)
    k = cv2.waitKey(1) & 0xFF
    if k == 27 or k == ord('q'): # angka 27 key dari esc
        break
print("EXIT")
cam.release()
cv2.destroyAllWindows()
